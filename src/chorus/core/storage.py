...
class PromptStorage:
    ...
    def add_prompt(self, prompt_version: PromptVersion) -> None:
        ...
        key = f"{prompt_version.function_name}_{prompt_version.version}"
        self.prompts[key] = prompt_version
        self._save_prompts()
    
    def get_prompt(self, function_name: str, version: str) -> Optional[PromptVersion]:
        """Get a specific prompt version."""
        key = f"{function_name}_{version}"
        return self.prompts.get(key)
    
    def get_latest_version(self, function_name: str) -> Optional[PromptVersion]:
        """Get the latest version of a prompt for a function."""
        versions = [pv for pv in self.prompts.values() if pv.function_name == function_name]
        if not versions:
            return None
        return max(versions, key=lambda x: self._parse_version(x.version))
